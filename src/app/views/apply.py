...
from models.user import get_user_name_by_id, get_user_id_by_name, get_users
...
class Apply:
    ...
    def GET(self):
        ...
        render = web.template.render('templates/', globals={"get_apply_permissions_form": get_apply_permissions_form,
                                                            'session': session})
    ...
    def POST(self):
        ...
        if data.projectid:
            ...
            if data.add_user:
                ...
                # if not applicants:
                #   return render.apply(nav, apply_form, "- User doesn't exist.", project, applicants, permissions)
                return render.apply(nav, apply_form, get_apply_permissions_form, project, applicants, permissions)
                ...
                for applicant, permission in zip(applicants, permissions):
                    models.project.set_projects_user(data.projectid, str(applicant[0]), permission[0], permission[1],
                                                     permission[2])
    ...
    def get_applicants(self, data, operation):
        ...
        if operation == "remove_user":
            ...
            users = get_users()
            if user_name_to_add not in ([user[1] for user in users]):
