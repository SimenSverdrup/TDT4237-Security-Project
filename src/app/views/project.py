...
class Project:
    ...
    def GET(self):
        ...
        if data.projectid:
            ...
            tasks = [[]]
        render = web.template.render('templates/',
                                     globals={'get_task_files': models.project.get_task_files, 'session': session})
    ...
    def POST(self):
        ...
        try:
            if fileitem.filename:
                ...
                format = fileitem.filename.split('.')[-1]
                accepted_formats = ['pdf', 'txt', 'docx', 'xlsx']
                if format not in accepted_formats:
                    ...
                    render = web.template.render('templates/', globals={'get_task_files': models.project.get_task_files,
                                                                        'session': session})
                    message = f"'{format}' is not an accepted format. Must be one of {accepted_formats}."
                    return render.project(nav, project_form, project, tasks, permissions, categories, message)
